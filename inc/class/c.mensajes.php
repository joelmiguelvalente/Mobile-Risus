...
class tsMensajes {
    ...
    */
    function newMensaje(){
		...
		if(db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT mp_id FROM `u_mensajes` WHERE (mp_date > \''.(time()-$antiflood).'\' && mp_from = \''.$tsUser->uid.'\') OR (mp_date > \''.(time()-$antiflood*3600).'\' && mp_from = \''.$tsUser->uid.'\' && mp_preview = \''.$mensajito.'\' && `mp_subject` = \''.$tsCore->setSecure($_POST['asunto']).'\') ORDER BY mp_id DESC LIMIT 1'))) die('0: Espere '.$antiflood.' segundos para continuar'); 
		...
		break;
			...
			if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `u_mensajes` (`mp_to`, `mp_from`, `mp_subject`, `mp_preview`, `mp_date`) VALUES ('.$user_id.', '.$tsUser->uid.', \''.$asunto.'\', \''.$preview.'\',\''.time().'\')')) {
				...
				if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `u_respuestas` (`mp_id`, `mr_from`, `mr_body`, `mr_ip`, `mr_date`) VALUES (\''.(int)$mp_id.'\', \''.$tsUser->uid.'\', \''.$mensaje.'\',  \''.$_SERVER['REMOTE_ADDR'].'\', \''.time().'\')')) {
                   return '1: '.$tsCore->settings['url'].'/mensajes/leer/'.$mp_id;
                } else return show_error('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db');
            } else return '0: Ocurri&oacute; un error. Int&eacute;ntalo nuevamente.';
		} else return '0: El usuario no puede recibir nuevos mensajes.';
		} else return '0: El usuario no existe. Int&eacute;ntalo nuevamente.';
		} else return '0: Debe tener una cuenta activa para realizar esta operaci&oacute;n';
    ...
    */
    function newRespuesta(){
        ...
        if(!empty($msg)){
            ...
            if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `u_respuestas` (`mp_id`, `mr_from`, `mr_body`, `mr_ip`, `mr_date`) VALUES (\''.intval($mp_id).'\', \''.$tsUser->uid.'\', \''.$mp_body.'\', \''.$_SERVER['REMOTE_ADDR'].'\', \''.time().'\')')){
            // CUANDO RESPONDA EL DESTINATARIO...
                ...
                db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE `u_mensajes` SET `mp_preview` = \''.$preview.'\', `mp_date` = \''.time().'\' '.$update.' WHERE `mp_id` = \''.(int)$mp_id.'\'');
                //
                $return['mp_date'] = time();
