         ...
         // INSERTAMOS EL PERFIL
			...
			$body = '<div style="background:#0f7dc1;padding:10px;font-family:Arial, Helvetica,sans-serif;color:#000">
				...
				<div style="background:#FFF;padding:10px;font-size:14px">
					...
					<p>Para finalizar con el proceso de registro, confirma tu direcci&oacute;n de email accediendo a <a href="'.$tsCore->settings['web'].'/validar/'.$key.'/2/'.$tsCore->setSecure($tsData['user_email']).'">este enlace</a>
					</p><br /> <br />
					...
					<p>Antes de empezar a interactuar con la comunidad, te recomendamos que visites el <a target="_blank" href="'.$tsCore->settings['web'].'/pages/protocolo/">Protocolo</a> del sitio.</p>
			...
			$tsEmail->sendEmail($from, $to, $subject, $body)  or die('0: Hubo un error al intentar procesar lo solicitado');				
						return '1: <div class="box_cuerpo" style="padding: 12px 20px;">Te hemos enviado un correo a <b>'.$to.'</b> con los &uacute;ltimos pasos para finalizar con el registro.<br><br>Si en los pr&oacute;ximos minutos no lo encuentras en tu bandeja de entrada, por favor, revisa tu carpeta de correo no deseado, es posible que se haya filtrado.<br><br>&iexcl;Muchas gracias!</div>';	
				}else{
