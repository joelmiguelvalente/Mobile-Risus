...
class tsMonitor {
    ...
    */
     ...
     */
    public function readAviso($av_id = NULL){
        # GLOBALES
        global $tsUser;
        # OBTENEMOS
     ...
     */
    public function delAviso($av_id = NULL){
        # GLOBALES
        global $tsUser;
        # OBTENEMOS
...
public function setNotificacion($type = NULL, $user_id = NULL, $obj_user = NULL, $obj_uno = 0, $obj_dos = 0, $obj_tres = 0){
    ...
    */
    private function makeOracion($data = NULL){
        ...
        switch($no_type){
            ...
            break;
            case 15:
                $oracion['text'] = 'Recibiste una nueva <span class="qtip" title="'.$data['m_title'].'"><b>medalla</b> <img src="'.$tsCore->settings['web'].'/themes/default/images/icons/med/'.$data['m_image'].'_16.png"/></span>';
                $oracion['user'] = $tsUser->nick;
                $oracion['avatar'] = $tsUser->uid.'_50.jpg';
            break;
            case 16:
                $oracion['text'] = 'Tu <a href="'.$site_url.'/posts/'.$data['c_seo'].'/'.$data['post_id'].'/'.$tsCore->setSEO($data['post_title']).'.html" title="'.$data['post_title'].'"><b>post</b></a> tiene una nueva <span class="qtip" title="'.$data['m_title'].'"><b>medalla</b> <img src="'.$tsCore->settings['web'].'/themes/default/images/icons/med/'.$data['m_image'].'_16.png"/></span>';
                $oracion['user'] = $tsUser->nick;
                $oracion['avatar'] = $tsUser->uid.'_50.jpg';
            break;
            case 17:
                $oracion['text'] = 'Tu <a href="'.$site_url.'/fotos/'.$data['user_name'].'/'.$data['foto_id'].'/'.$tsCore->setSEO($data['f_title']).'.html" title="'.$data['f_title'].'"><b>foto</b></a> tiene una nueva <span class="qtip" title="'.$data['m_title'].'"><b>medalla</b> <img src="'.$tsCore->settings['web'].'/themes/default/images/icons/med/'.$data['m_image'].'_16.png"/></span>';
                $oracion['user'] = $tsUser->nick;
                $oracion['avatar'] = $tsUser->uid.'_50.jpg';
            break;
        }
    ...
    */
    public function setFollow(){
        ...
        if(empty($data['follow_id'])){
            if($tsUser->uid == $fw['obj'] && $fw['type'] == 1) return '0: No puedes seguirte a ti mismo.';
            if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `u_follows` (`f_user`, `f_id`, `f_type`, `f_date`) VALUES (\''.$tsUser->uid.'\', \''.(int)$fw['obj'].'\', \''.$fw['type'].'\', \''.time().'\')')){
                ...
                // RESPUESTA
                return '1: bien!';
            } else return '0: No se pudo completar la acci&oacute;n.';
        } else return '0: Ya sigues a este usuario.';
    }
    /**
     * @name setUnFollow
     * @access public
    ...
    */
    public function setSpam(){
        ...
        //
        $postid = $tsCore->setSecure($_POST['postid']);
        // TIENE SEGUIDORES?
