...
class tsUser {
	...
	*/
	function loginUser($username, $password, $remember = TRUE, $redirectTo = NULL){
		global $tsCore;
		...
		/* CONSULTA */
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT user_id, user_password, user_pwtype, user_activo, user_baneado FROM u_miembros WHERE LOWER(user_name) = \''.$username.'\' LIMIT 1');
        if(!$query) $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT user_id, user_password, user_activo, user_baneado FROM u_miembros WHERE LOWER(user_name) = \''.$username.'\' LIMIT 1');
        //
...
class tsSession {
	 ...
	 * @return	void
	 */
	function update($user_id = 0, $autologin = TRUE, $force_update = FALSE)
	{
       ...
       $this->userdata['session_autologin'] = 1;
