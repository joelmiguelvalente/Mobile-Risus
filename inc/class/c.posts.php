...
class tsPosts {
	...
	*/
    function getLastPosts($category = NULL, $time = 0, $start = 0, $limit = 25) {
      ...
      $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT p.post_id, p.post_body, p.post_user, p.post_category, p.post_title, p.post_date, p.post_comments, p.post_puntos, p.post_private, p.post_sponsored, p.post_status, p.post_sticky, u.user_id, u.user_name, u.user_activo, u.user_baneado, c.c_nombre, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id  '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : ' && u.user_activo = \'1\' && u.user_baneado = \'0\'').' LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? 'p.post_id > 0' : 'p.post_status = \'0\' && u.user_activo = \'1\' && u.user_baneado = \'0\'').' '.$c_where.' AND p.post_sticky = \'0\' '.$time.' GROUP BY p.post_id ORDER BY '.$order.' DESC LIMIT '.$start.','.$limit);
      $lastPosts['data'] = result_array($query);
	  	$lastPosts['total'] = count($lastPosts['data']);
	  // COVER
	  $i = 0;
	  foreach($lastPosts['data'] as $row){		
		  $lastPosts['data'][$i]['post_cover'] = $tsCore->extraer_img($lastPosts['data'][$i]['post_body']);
		  $i++;
	  }
	...
	*/
	...
	/*
		getRelated()
	*/
	function getRelated($tags = NULL, $start = 0){
		global $tsCore, $tsUser;
		...
		foreach($data['data'] as $row){		
			$data['data'][$i]['post_cover'] = $tsCore->extraer_img($data['data'][$i]['post_body']);
			$i++;
		}
		//
		return $data;
	...
	/*
		getComentarios()
	*/
	function getComentarios($post_id = NULL, $start = 0){
		global $tsCore, $tsUser;
	...
	*/
	function getFavoritos($start = 0, $limit = 25){
		global $tsCore, $tsUser;
		//
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT p.post_id, p.post_body, p.post_title, p.post_puntos, p.post_comments, c.c_seo, c.c_img FROM p_favoritos AS f LEFT JOIN p_posts AS p ON p.post_id = f.fav_post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE f.fav_user = \''.$tsUser->uid.'\' AND p.post_status = \'0\' ORDER BY f.fav_date DESC LIMIT '.$start.','.$limit);
		$data['data'] = result_array($query);
		//
		$data['total'] = count($data['data']);		
		// COVER
