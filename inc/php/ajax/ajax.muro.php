	...
	switch($action){
		case 'muro-stream':
            ...
            if($do == 'post'){
                ...
                //
				$tsStream = ($_GET['act'] == 'portal')? $tsMuro->getNews($start) : $tsMuro->getWall($user_id,$start);
				...
				$tsPage = 'php_files/p.comentario.preview'; // TEMPLATE
                // VARIABLES
                ...
                if(!is_array($tsRepost)) {
                    ...
                    $smarty->assign("tsComment",$tsRepost);  
                }
            }
		break;
        case 'muro-likes':
